...
import { useAllRemarks } from "../../shared/hooks/useRemarks";
...
export const RemarksBlock = ({ data }: RemarksBlockData) => {
  const [activeButton, setActiveButton] = useState<ButtonKey>("all");
  const navigate = useNavigate();
  const { data: remarks } = useAllRemarks(data?.id ?? "");
          ...
          return (
            ...
            >
              ...
              <div className="px-[16px] font-[600] text-[16px] leading-[24px] tracking-[-0.2px] text-[#413F3F]">
                {formatDate(row.date_remark)} |{" "}
                {format(parseISO(row.date_remark), "HH:mm")}
              </div>
              <div className="px-[16px] font-[600] text-[16px] leading-[24px] tracking-[-0.2px] text-[#413F3F]">
              ...
              {row.status !== "active" && (
                <div className="px-[16px] flex justify-end">
                  <Button
                    ...
                    onClick={() =>
                      navigate(
                        `/objects/remark/${row.object_name}?id=${row.id}`
                      )
                    }
                  />
                </div>
