...
import {
  useChangeViolationStatus,
  useViolationById,
} from "../../shared/hooks/useViolations";
...
export const ViolationPage = () => {
  ...
  const { data: violation } = useViolationById(id);
  const { mutate: agree } = useChangeViolationStatus();
  const { data: user } = useCurrentUser();
  const navigate = useNavigate();
  const { formatDate } = useFormatDate();

  const { statusText, statusColor } = dataMap[violation?.status ?? "not_fixed"];
  ...
  const handleAgree = (action: "accept" | "deny") => {
    agree(
      { violationId: violation?.id ?? "", action },
      { onSuccess: () => navigate("/objects") }
    );
  };

  const time = violation?.date_violation
    ? format(parseISO(violation?.date_violation), "HH:mm")
    : "";
